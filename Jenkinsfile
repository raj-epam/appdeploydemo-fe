    ...
    stages {
        stage ('checkout') {
                    steps {
                        ...
                        dir("build")
                        {
                            ...
                            url: 'https://github.com/raj-epam/appdeploydemo-be.git'
                        }
                    }
                }
        ...
        stage ("Test Code") {
            steps {
                dir("build/tests") {
                    ...
                    // Activate the virtual environment
                    sh '. venv/bin/activate && pip install -r requirements.txt && pip list'
                    sh 'pip install -r requirements.txt '
                    //def scannerHome = tool 'sonarqube'
                    //env.PATH = "${scannerHome}/bin:${env.PATH}"
                //     }
        ...
        stage ('Build') {
                steps {
                    dir("build")
                    {
                        sh 'docker build -t dhlawowacr.azurecr.io/pybeapi:${BUILD_NUMBER} .'
                    }
                }
        }
                ...
                script{
                    dir("build")
                        {   
                            ...
                            echo "Starting Scan"
                            def scanExitCode = sh(script: 'snyk container test dhlawowacr.azurecr.io/pybeapi:${BUILD_NUMBER}  --file=./Dockerfile --json-file-output=vulnerabilities.json --org=raj-epam --project-name=python_demo-FE', returnStatus: true)
                            // def display = sh(script: 'snyk container monitor dhlawowacr.azurecr.io/pybeapi:${BUILD_NUMBER}  --file=./Dockerfile --org=raj-epam --project-name=python_demo-FE --project-environment=frontend', returnStatus: true)
        ...
        stage('Run DAST Scan') {
            steps {
                script {
                    ...
                    def dockerRunCmd = """docker run -d --name my-python-app -p 80:80 dhlawowacr.azurecr.io/pybeapi:${BUILD_NUMBER}"""
                    sh dockerRunCmd
        ...
        stage('Push Image') {//Manifest Deploy for Dev Env Values
            ...
            { withCredentials([string(credentialsId: 'svc_principal', variable: 'PASSWORD')]) {
                ...
                sh 'docker push dhlawowacr.azurecr.io/pybeapi:${BUILD_NUMBER}'       
                sh 'docker image rm dhlawowacr.azurecr.io/pybeapi:${BUILD_NUMBER}'     
        ...
        //             sh "sed -i 's|dhlawowacr.azurecr.io/pybeapi:[^\"]*|dhlawowacr.azurecr.io/pybeapi:${BUILD_NUMBER}|g' deployment.yaml"
